...
function Assert-ReleaseFrontend {
...
function Get-PayloadReport {
    param([string]$repoRoot, [string]$exePath)
    $pdfiumPath = Join-Path $repoRoot 'src-tauri\resources\pdfium\pdfium.dll'
    $payloadCheck = Join-Path $PSScriptRoot 'check-payload.mjs'
    $payloadJson = node $payloadCheck $exePath $pdfiumPath
    if ($LASTEXITCODE -ne 0) { throw 'Installer payload validation failed. No installer was compiled.' }
    return $payloadJson
}
...
function Invoke-InnoCompilation {
    param([string]$iscc, [string]$version, $payload, [string]$issPath, [string]$outputDir)
    ...
    while (-not $compiled -and $attempt -lt $maxAttempts) {
        $attempt++
        & $iscc "/DAppVersion=$version" "/DExeSha256=$($payload.exe.sha256)" "/DPdfiumSha256=$($payload.pdfium.sha256)" $issPath
...
$tauriConf = Get-Content -LiteralPath (Join-Path $repoRoot 'src-tauri\tauri.conf.json') -Raw | ConvertFrom-Json
$version = $tauriConf.version
Write-Host "Abakus version: $version"
if (-not $SkipBuild) { Invoke-AppBuild $repoRoot }
$exePath = Join-Path $repoRoot 'target\release\abakus.exe'
Assert-ReleaseFrontend $repoRoot $exePath
$payloadJson = Get-PayloadReport $repoRoot $exePath
$payload = ($payloadJson -join "`n") | ConvertFrom-Json
$iscc = Find-InnoCompiler
$issPath = Join-Path $PSScriptRoot 'abakus.iss'
$outputDir = Join-Path $PSScriptRoot 'output'
if (-not (Test-Path -LiteralPath $outputDir)) {
    New-Item -ItemType Directory -Path $outputDir | Out-Null
}
$payloadJson | Set-Content -LiteralPath (Join-Path $outputDir "abakus-payload-$version.json") -Encoding UTF8
Invoke-InnoCompilation $iscc $version $payload $issPath $outputDir
